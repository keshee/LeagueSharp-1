...
namespace Riven
{
    class Program
    {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Config.Item("ComboActive").GetValue<KeyBind>().Active)
            {
                ...
                if (target != null)
                {
                    ...
                    var noAAComboDamage = GetNoAAComboDamage(target);
                    bool useRComboToKill = false;

                    if(noAAComboDamage >= target.Health && !Buffmanager.ROn && Vector3.Distance(target.ServerPosition, ObjectManager.Player.ServerPosition) <= E.Range + (target.BoundingRadius/2))
                    {
                        Game.PrintChat("Using no AA combo to quickly destroy"); //Damage calc seems off (need to check into this)
                        if(E.IsReady())
                        {
                    ...
                    if (Buffmanager.windSlashReady)
                    {
                        ...
                        if (rdamage >= target.Health && Vector3.Distance(ObjectManager.Player.ServerPosition, target.ServerPosition) <= R.Range)
                        {
                    ...
                    if(useRComboToKill)
                    {
                        if (useECombo && E.IsReady() && useRCombo && R.IsReady() && !Orbwalking.InAutoAttackRange(target) && Vector3.Distance(target.ServerPosition, ObjectManager.Player.ServerPosition) < E.Range + target.BoundingRadius + 75)
                        {
                            E.Cast(target.ServerPosition, true);
                            ObjectManager.Player.Spellbook.CastSpell(SpellSlot.R);
                        }
                    ...
                    if(!useRComboToKill)
                    {
                        if (useECombo && E.IsReady() && !Orbwalking.InAutoAttackRange(target) && Vector3.Distance(target.ServerPosition, ObjectManager.Player.ServerPosition) < E.Range + target.BoundingRadius)
                        {
                            E.Cast(target.ServerPosition, true);
                            ObjectManager.Player.Spellbook.CastSpell(SpellSlot.R);
                        }
                    ...
                    if(ROn)
                    {
                        if (useWCombo && W.IsReady() && Vector3.Distance(ObjectManager.Player.ServerPosition, target.ServerPosition) 
        ...
        private static void Game_OnSendPacket(GamePacketEventArgs args)
        {
            try
            {
                if (args.PacketData[0] == 0x9A && Orbwalker.ActiveMode.ToString() == "Combo")
                {
        ...
        { //Check into packet 0x104 also move Q cast to Process spell and verify with packet
            try
            {
                if (Orbwalker.ActiveMode.ToString() == "Combo")
                {
                    if (args.PacketData[0] == 0x65 && Q.IsReady())
                    {
